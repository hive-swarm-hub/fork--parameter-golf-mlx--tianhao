#!/usr/bin/env bash
# Evaluate train_gpt_mlx.py: run training, parse metrics, output summary.
...
# 1. Check train_gpt_mlx.py exists
if [ ! -f "train_gpt_mlx.py" ]; then
    echo "ERROR: train_gpt_mlx.py not found." >&2
    summary "ERROR" "0" "0" "false"
    exit 0
fi

# 2. Check line count
LINE_COUNT=$(wc -l < train_gpt_mlx.py)
if [ "$LINE_COUNT" -gt 1500 ]; then
    echo "ERROR: train_gpt_mlx.py has $LINE_COUNT lines (limit: 1500)." >&2
    summary "ERROR" "0" "$LINE_COUNT" "false"
    exit 0
fi

# 3. Check MLX available
if ! python3 -c "import mlx.core" 2>/dev/null; then
    echo "ERROR: MLX not available." >&2
    summary "ERROR" "0" "$LINE_COUNT" "false"
    exit 0
fi
...
echo "Running: python3 train_gpt_mlx.py" >&2

TRAIN_EXIT=0
python3 train_gpt_mlx.py 2>&1 | tee "$TMPLOG" >&2 || TRAIN_EXIT=$?
...
# Extract artifact bytes from "serialized_model_int8_zlib:" line
ARTIFACT_BYTES=$(grep -oP 'serialized_model_int8_zlib:\K[0-9]+' "$TMPLOG" | tail -1)
