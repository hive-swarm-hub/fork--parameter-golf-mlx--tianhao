# Parameter Golf (MLX) — Train the Best Language Model in 16MB

Improve an MLX-based GPT training script to minimize `val_bpb` (bits-per-byte on FineWeb validation) while fitting within a 16MB artifact budget. Runs on Apple Silicon via MLX.

## Setup

1. **Read the in-scope files**:
   - `train_gpt_mlx.py` — the file you modify. The GPT training script (MLX).
...
The challenge: train the best language model that fits in **16MB** (code + int8+zlib compressed model), trained in **≤10 minutes** on an Apple Silicon Mac (MLX).

- **Metric**: `val_bpb` — bits-per-byte on FineWeb validation data. **Lower is better.**
- **Artifact limit**: Code size + compressed model (`*_mlx_model.int8.ptz`) ≤ 16,000,000 bytes
- **Script limit**: `train_gpt_mlx.py` must be ≤ 1500 lines
- **Training cap**: 10-minute wallclock limit (enforced by `MAX_WALLCLOCK_SECONDS=600` in the script)
- **Baseline**: ~1.2244 val_bpb (approximate — may vary on different Apple Silicon hardware)

## Experimentation

**What you CAN modify:**
- `train_gpt_mlx.py` — everything is fair game: model architecture, optimizer, learning rate schedule, hyperparameters, tokenizer usage, quantization strategy, data loading, sequence length, batch size, number of layers/heads/width, activation functions, normalization, weight tying, etc.
...
- `line_count`: number of lines in train_gpt_mlx.py
...
2. Modify `train_gpt_mlx.py` with your experiment.
3. git commit
